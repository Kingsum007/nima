    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header">
                ...
                <a href="{{route('semester.create')}}" class="btn btn-primary float-end"><i class="fa fa-plus"></i></a>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <table id="example1" class="table table-bordered table-striped dataTable dtr-inline"
                                ...
                                <tbody>
                                    @foreach ($semeseter as $student)
                                        <tr class="odd">
                                            <td>{{ $student->id }}</td>
                                            <td>{{ $student->sem_title }}</td>
                                            <td>{{ $student->dept_title }}</td>
                                            <td>{{ $student->sem_time }}</td>
                                            <td><a href="{{ route('semester.edit', [$student->id]) }}"
                                                    class="btn  btn-info"><i class="fas fa-edit"></i></a>
                                                    <form action="{{route('semester.destroy',[$student->id])}}" method="post">
                                                        @method('DELETE')
                                                        @csrf
